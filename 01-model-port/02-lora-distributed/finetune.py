...
def get_model_and_tokenizer(model_name, use_lora, token, inference=False, device_map="auto"):
    if inference:
        if use_lora:
            model = AutoPeftModelForCausalLM.from_pretrained(
                model_name, token=token, torch_dtype=torch.bfloat16, device_map=device_map
            )
        else:
...
def main(training_args, det_callback, hparams):
    ...
    token = hparams["token"]
    model, tokenizer = get_model_and_tokenizer(model_name, hparams["lora"], token)
